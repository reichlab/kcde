...
kcde <- function(X_names,
    ...
    param_estimates <- est_kcde_params_stepwise_crossval(
        data = data,
...
est_kcde_params_stepwise_crossval <- function(data,
                                              ...
                                              init_theta_vector,
                                              init_phi_vector) {
    all_vars_and_offsets <-
        plyr::rbind.fill(lapply(kcde_control$kernel_components,
                                function(kernel_component) {
                                    kernel_component$vars_and_offsets
                                }))
    predictive_vars_and_offsets <- all_vars_and_offsets[all_vars_and_offsets$offset_type == "lag", , drop = FALSE]
    ...
    current_model_vars_and_offsets <- all_vars_and_offsets[all_vars_and_offsets$offset_type == "horizon", , drop = FALSE]
    theta_hat <-
        vector("list", length(kcde_control$kernel_components))
    ...
    if (identical(kcde_control$variable_selection_method, "stepwise")) {
        repeat {
            ...
            crossval_results <-
                ...
                function(i) {
                    ...
                    if (!model_i_previously_evaluated &&
                        any(descriptor_updated_model$offset_type == "lag")) {
            ...
            crossval_results <-
                crossval_results[non_null_components]

            all_evaluated_models <-
                c(all_evaluated_models, crossval_results)
            ...
            loss_achieved <-
                sapply(crossval_results, function(component) {
                    component$loss
                })
            ...
            if (loss_achieved[optimal_loss_ind] < crossval_prediction_loss) {
                ...
                crossval_prediction_loss <-
                    loss_achieved[selected_var_lag_ind]
                current_model_vars_and_offsets <-
                    crossval_results[[selected_var_lag_ind]]$vars_and_offsets
                theta_hat <-
                    crossval_results[[selected_var_lag_ind]]$theta
                phi_hat <-
                    crossval_results[[selected_var_lag_ind]]$phi
            } else {
    ...
    return(
        list(
            vars_and_offsets = current_model_vars_and_offsets,
            theta_hat = theta_hat,
            phi_hat = phi_hat,
...
est_kcde_params_stepwise_crossval_one_potential_step <- function(prev_vars_and_offsets,
                                                                 prev_theta,
                                                                 prev_phi,
                                                                 init_theta_vector,
                                                                 init_phi_vector,
    ...
    max_filter_window_size <- suppressWarnings(max(unlist(
        lapply(kcde_control$filter_control,
    ...
    max_lag <- max(unlist(lapply(kcde_control$kernel_components,
                                 function(kernel_component) {
                                     kernel_component$vars_and_offsets$offset_value[kernel_component$vars_and_offsets$offset_type == "lag"]
                                 })))

    max_horizon <- max(unlist(lapply(kcde_control$kernel_components,
                                     function(kernel_component) {
                                         kernel_component$vars_and_offsets$offset_value[kernel_component$vars_and_offsets$offset_type == "horizon"]
                                     })))
    ...
    theta_optim_bounds <- get_theta_optim_bounds(theta = theta_init,
                                                 kcde_control = kcde_control)
    optim_result <- optim(
        par = c(phi_est_init, theta_est_init),
    ...
    temp <-
        update_phi_from_vectorized_phi_est(
            phi_est_vector = optim_result$par,
            phi = phi_init,
            filter_control = kcde_control$filter_control
        )
    ...
    updated_theta <- update_theta_from_vectorized_theta_est(optim_result$par[seq(from = next_param_ind,
...
kcde_crossval_estimate_parameter_loss <-
    function(combined_params_vector,
             phi,
             theta,
             vars_and_offsets,
             data,
        ...
        temp <-
            update_phi_from_vectorized_phi_est(combined_params_vector,
                                               phi,
        ...
        theta <- update_theta_from_vectorized_theta_est(combined_params_vector[seq(from = next_param_ind,
        ...
        predictive_var_combined_names <-
            vars_and_offsets$combined_name[vars_and_offsets$offset_type == "lag"]
        target_var_combined_names <-
            vars_and_offsets$combined_name[vars_and_offsets$offset_type == "horizon"]
        ...
        num_obs_per_crossval_group <-
            floor(nrow(cross_validation_examples) /
        ...
        t_train_groups <- lapply(seq_len(kcde_control$par_cores),
                                 function(group_num) {
                                     if (!identical(group_num, as.integer(kcde_control$par_cores))) {
                                         return(
                                             seq(
                                                 from = (group_num - 1) * num_obs_per_crossval_group + 1,
        ...
        kernel_values_by_t_train_group <-
            foreach(
                i = seq_along(t_train_groups),
                .packages = c("kcde", kcde_control$par_packages),
                .combine = "c"
            ) %dopar% {
                ...
                for (t_train_ind in seq_along(t_train_groups[[i]])) {
                    ...
                    if (is.null(kcde_control$time_name)) {
                        ...
                        t_train_near_t_pred <-
                            abs(t_pred - t_train) <= as.integer(kcde_control$crossval_buffer)
                    } else {
                        ## If there is a time variable, we compute indices to leave out based on that
                        train_time <-
                            cross_validation_examples[t_train, kcde_control$time_name]
                        t_train_near_t_pred <-
                    ...
                    train_lagged_obs <- cross_validation_examples[t_train, predictive_var_combined_names, drop =
                                                                      FALSE]
                    train_lead_obs <- cross_validation_examples[t_train, target_var_combined_names, drop =
                                                                    FALSE]
                    prediction_lagged_obs <-
                        cross_validation_examples[t_pred, predictive_var_combined_names, drop =
                                                      FALSE]
                    prediction_lead_obs <-
                        cross_validation_examples[t_pred, target_var_combined_names, drop =
                                                      FALSE]
                    ...
                    for (target_var_ind in seq_along(target_var_combined_names)) {
                        target_var_combined_name <-
                            target_var_combined_names[target_var_ind]
        ...
        for (group_results in kernel_values_by_t_train_group) {
            ...
            kernel_values_by_time_pair[, seq(from = kv_row_ind, length = nrow_group_results),] <-
                group_results
        ...
        log_sum_kernel_vals_by_kernel_var <- lapply(seq_len(dim(kernel_values_by_time_pair)[3]),
                                                    function(target_var_ind) {
                                                        return(logspace_sum_matrix_rows(as.matrix(kernel_values_by_time_pair[, , target_var_ind])))
                                                    })
        ...
        if (length(log_sum_kernel_vals_by_kernel_var) > 2L) {
            for (target_var_ind in seq(from = 3,
                                       to = length(log_sum_kernel_vals_by_kernel_var))) {
        ...
        crossval_loss_eq_0 <- sapply(crossval_loss_by_time_ind,
                                     function(cvli) {
                                         isTRUE(all.equal(cvli, 0))
                                     })
        if (any(
            is.na(crossval_loss_by_time_ind) |
