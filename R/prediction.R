...
kcde_predict <- function(kcde_fit,
    ...
    training_non_na_rows <-
        which(!apply(training_examples[kcde_fit$vars_and_offsets$combined_name],
                     1,
                     anyNA))

    training_examples <-
        training_examples[training_non_na_rows, , drop = FALSE]
    ...
    prediction_examples <-
        prediction_examples[nrow(prediction_examples), , drop = FALSE]

    predictive_var_combined_names <-
        kcde_fit$vars_and_offsets$combined_name[kcde_fit$vars_and_offsets$offset_type == "lag"]
    target_var_combined_names <-
        kcde_fit$vars_and_offsets$combined_name[kcde_fit$vars_and_offsets$offset_type == "horizon"]
...
kcde_predict_given_lagged_obs <- function(train_lagged_obs,
    ...
    if (identical(prediction_type, "centers-and-weights")) {
        ...
        return(
            kcde_point_predict_given_kernel_centers_and_weights(
        ...
        return(
            kcde_sample_predict_given_lagged_lead_obs(
                n = n,
                train_lagged_obs = train_lagged_obs,
                train_lead_obs = train_lead_obs,
                prediction_lagged_obs = prediction_lagged_obs,
        ...
        return(
            kcde_quantile_predict_given_lagged_lead_obs(
                p = p,
                n = n,
                train_lagged_obs = train_lagged_obs,
                train_lead_obs = train_lead_obs,
        ...
        return(
            kcde_prob_predict_given_lagged_lead_obs(
                q = q,
                n = n,
                train_lagged_obs = train_lagged_obs,
                train_lead_obs = train_lead_obs,
...
kcde_kernel_centers_and_weights_predict_given_lagged_obs <-
    function(train_lagged_obs,
        ...
        log_weights <-
            compute_normalized_log_weights(unnormalized_log_weights)
...
kcde_dist_predict_given_lagged_lead_obs <-
    function(train_lagged_obs,
        ...
        ## of lagged observation vectors from train_data and predict_data.
        ## result is a vector of length nrow(train_lagged_obs)
        log_kernel_values_x <- compute_kernel_values(
            train_lagged_obs,
            prediction_lagged_obs,
            kernel_components = kcde_fit$kcde_control$kernel_components,
            theta = kcde_fit$theta_hat,
        ...
        log_result <-
            sapply(seq_len(nrow(prediction_test_lead_obs)),
...
#'     centers: a copy of the train_lead_obs argument -- kernel centers
kcde_point_predict_given_kernel_centers_and_weights <-
    function(kernel_centers_and_weights,
             kcde_fit) {
...
kcde_sample_predict_given_lagged_lead_obs <- function(n,
    ...
    sampled_kernel_inds <-
        sample(
            length(kernel_centers_and_weights$weights),
            size = n,
            replace = TRUE,
    ...
    for (kernel_ind in unique(sampled_kernel_inds)) {
        ...
        result[result_inds,] <-
            simulate_values_from_product_kernel(
                n = length(result_inds),
                conditioning_obs = prediction_lagged_obs,
                center = cbind(
                    kernel_centers_and_weights$conditioning_vars[kernel_ind, , drop = FALSE],
                    kernel_centers_and_weights$centers[kernel_ind, , drop = FALSE]
                ),
                kernel_components = kcde_fit$kcde_control$kernel_components,
                theta = kcde_fit$theta
            )[, colnames(kernel_centers_and_weights$centers)]
    }
...
kcde_quantile_predict_given_lagged_lead_obs <- function(p,
                                                        n,
                                                        train_lagged_obs,
                                                        train_lead_obs,
                                                        prediction_lagged_obs,
...
kcde_prob_predict_given_lagged_lead_obs <- function(q,
                                                    n,
                                                    train_lagged_obs,
                                                    train_lead_obs,
                                                    prediction_lagged_obs,
